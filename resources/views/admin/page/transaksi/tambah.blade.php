...
<section class="content">
    <div class="card card-info">
      ...
      <form class="form-horizontal" method="post" 
        ...
        <div class="card-body">
          <div class="form-group row">
           ...
           <span class="text-info">
            <i class="fas fa-user-circle"></i> <u>Data Transaksi</u>
            </span></label>
          </div>        
        ...
        User_pengguna</label>
        <div class="col-sm-7">
          <select class="form-control @error('user_pengguna') 
      is-invalid @enderror" id="user_pengguna" name="user_pengguna">
            <option value="">- Pilih User Pengguna -</option>
            @if (!empty($DataUserPengguna))
                @foreach($DataUserPengguna as $key => $UserPengguna)
                    <option value="{{ $UserPengguna->id_user_pengguna }}">{{ $UserPengguna->nama }}</option>
                @endforeach
            @endif
          </select>
        </div>
      ...
      @error('jadwal')
        ...
        Jadwal</label>
        <div class="col-sm-7">
          <select class="form-control @error('jadwal') 
      is-invalid @enderror" id="jadwal" name="jadwal">
      ...
      @enderror
          <div class="form-group row">
            ...
            <div class="col-sm-7">
              <input type="text" class="form-control 
                @error('total_tiket') is-invalid @enderror" name="total_tiket" 
                id="total_tiket" value="">
              </div>
          </div>
        ...
        <label for="harga" class="col-sm-3 col-form-label">
          ...
          <div class="col-sm-7">
          <input type="text" class="form-control 
          @error('total_harga') is-invalid @enderror" name="total_harga" 
      ...
      @error('notifikasi')
        ...
        Notifikasi</label>
        <div class="col-sm-7">
          <select class="form-control @error('notifikasi') 
      is-invalid @enderror" id="notifikasi" name="notifikasi">
            <option value="">- Pilih Notifikasi -</option>
            @if (!empty($DataNotifikasi))
                @foreach($DataNotifikasi as $key => $Notifikasi)
                    <option value="{{ $Notifikasi->id_notifikasi }}">{{ $Notifikasi->status }}</option>
                @endforeach
            @endif
          </select>
        </div>
          ...
          <div class="form-group row">
            <label for="tiket" class="col-sm-3 col-form-label">
            Tiket</label>
            <div class="col-sm-7">
            @if (!empty($DataTiket))
                @foreach($DataTiket as $key => $Tiket)
                    <input type="checkbox" name="list_tiket[]" 
                    value="{{ $Tiket->id_tiket }}"> {{ $Tiket->harga }} 
                    <br>
                @endforeach
            @endif
            </div>
